...
class AttentionLayer(Layer):
    ...
    def create_attention_mask(from_shape, input_mask):
        ...
        ones = tf.expand_dims(
            tf.ones(shape=from_shape[:2], dtype=tf.float32), axis=-1
        )  # [B, F, 1]
    ...
    def build(self, input_shape):
        ...
        self.value_layer = keras.layers.Dense(
            units=dense_units,
    ...
    def call(self, inputs, mask=None, training=None, **kwargs):
        ...
        attention_mask = AttentionLayer.create_attention_mask(
        ...
        batch_size, from_seq_len, from_width = (
            input_shape[0],
            input_shape[1],
            input_shape[2],
        )
        ...
        query = self.query_layer(
            from_tensor
        )  # [B,F, N*H] [batch_size, from_seq_len, N*H]
        ...
        if attention_mask is not None:
            ...
            # {1, 0} -> {0.0, -inf}
            adder = (
                1.0 - tf.cast(attention_mask, tf.float32)
            ) * self.params.negative_infinity
        ...
        attention_probs = self.dropout_layer(
            attention_probs, training=training
        )  # [B, N, F, T]

        # [B,T,N,H]
        value = tf.reshape(
            value,
            [batch_size, to_seq_len, self.params.num_heads, self.params.size_per_head],
        )
