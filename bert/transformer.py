...
class TransformerSelfAttentionLayer(Layer):
    class Params(ProjectionLayer.Params, AttentionLayer.Params):
        hidden_size = None
        num_heads = None
        hidden_dropout = None
    ...
    def _construct(self, **kwargs):
        ...
        if params.hidden_size % params.num_heads != 0:
            raise ValueError(
                "The hidden_size:[{}] is not a multiple of num_heads:[{}]".format(
                    params.hidden_size, params.num_heads
                )
            )
        self.size_per_head = params.hidden_size // params.num_heads
        assert (
            params.size_per_head is None or self.size_per_head == params.size_per_head
        )
    ...
    def call(self, inputs, mask=None, training=None):
        ...
        attention_output = self.attention_projector(
            [attention_head, layer_input], mask=mask, training=training
        )
...
class SingleTransformerEncoderLayer(Layer):
    ...
    def call(self, inputs, mask=None, training=None):
        ...
        attention_output = self.self_attention_layer(
            layer_input, mask=mask, training=training
        )
...
class TransformerEncoderLayer(Layer):
    ...
    def build(self, input_shape):
        ...
        if self.params.shared_layer:
            # ALBERT: share params
            self.shared_layer = SingleTransformerEncoderLayer.from_params(
                self.params, name="layer_shared"
            )
        else:
            # BERT
    ...
    def call(self, inputs, mask=None, training=None):
        ...
        for layer_ndx in range(self.params.num_layers):
            encoder_layer = (
                self.encoder_layers[layer_ndx]
                if self.encoder_layers
                else self.shared_layer
            )
